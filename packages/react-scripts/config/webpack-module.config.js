...
const { Externals } = require('sg1-loader');
...
module.exports = function(webpackEnv, appPackageJson) {
  const isEnvDevelopment = webpackEnv === 'development';
  const isEnvProduction = webpackEnv === 'production';
  const moduleId = appPackageJson.name;
  ...
  // sg1 config
  let sg1Config = appPackageJson.sg1Config;
  if (sg1Config) {
  ...
  const manifest = manifest => {
    ...
    for (let file in manifest) {
      ...
      if (isEnvProduction) {
        output[moduleId][key] = '/' + moduleId + manifest[file];
      } else {
        output[moduleId][key] = manifest[file];
      }
    }
  ...
  if (isEnvDevelopment) {
    // Include the default webpack-dev-server client
    const liveReloading = [
      'webpack-dev-server/client',
      'webpack/hot/only-dev-server',

      // DO NOT use the default CRA webpack client
      // It currently uses window.location.port which will not work
      // with modules being loaded by sg1
      // require.resolve('react-dev-utils/webpackHotDevClient'),
    ];

    // We only need to include the client once
    // The entire /src directory is being watched
